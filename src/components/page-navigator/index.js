...
import { NUMBER_OF_ANIMAL_STAT_SCREENS } from "../../constants";
...
const PageNavigator = ({ direction }) => {
  ...
  const shouldShowDownArrow =
    (!state.isAnimalStatsScreen || isLastAnimalStatScreen) && !isLastScreen;
  const handleClick = () => {
    goToNextSection();
  };
  ...
  const goToNextSection = () => {
    dispatch({
      type: direction === "down" ? "NEXT_SCREEN" : "NEXT_ANIMAL_STAT",
    });
  };
